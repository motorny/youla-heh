...
from keras.layers import Activation, Dense, Dropout, Input, concatenate
...
class CNN:
    ...
    def __init__(self):
        if self.__instance is None:
            ...
            data_positive = pd.read_csv(
                "./youla/model/negative.csv",
                sep=";",
                error_bad_lines=False,
                names=n,
                usecols=["text"],
            )
            data_negative = pd.read_csv(
                "./youla/model/negative.csv",
                sep=";",
                error_bad_lines=False,
                names=n,
            ...
            raw_data = np.concatenate(
                (
                    data_positive["text"].values[:sample_size],
            ...
            for size, filters_count in [(2, 10), (3, 10), (4, 10), (5, 10)]:
                for i in range(filters_count):
                    branch = Conv1D(
                        filters=1, kernel_size=size, padding="valid", activation="relu"
                    )(x)
    ...
    def run_model(self, array_str):
        ...
        return (
            int(sum(np.asarray(predicted))[0]),
            len(predicted) - int(sum(np.asarray(predicted))[0]),
            count_spam,
        )

    def detect_spam(self, array_str):
        data = [
            self.translator.translate(elem, src="ru", dest="en").text
            for elem in array_str
        ]
